...
export default {
  ...
  methods: {
    ...
    render: function (app_slug, id) {
      ...
      AppBuilder.get(app_slug).then(function (app) {
        ...
        AppBuilder.getRecord(app.slug, id).then(function (record) {
          ...
          if (typeof onEditPageLoaded[self.app.slug] === "function") {
            onEditPageLoaded[self.app.slug](self);
          }
        });
      });
    },
    save: function () {
      ...
      AppBuilder.updateRecord(self.app, self.values).then(function (result) {
        if (result) {
          if (typeof onRecordEdited[self.app.slug] === "function") {
            if (onRecordEdited[self.app.slug](self)) {
              self.$router.push("/app/view/" + self.app.slug);
            }
          } else {
            self.$router.push("/app/view/" + self.app.slug);
